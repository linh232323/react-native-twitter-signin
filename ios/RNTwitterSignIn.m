                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    [[Twitter sharedInstance] logInWithCompletion:^(TWTRSession * _Nullable session, NSError * _Nullable error) {
        if (error) {
            ...
            NSString *verifyCredentialsEndpoint = @"https://api.twitter.com/1.1/account/verify_credentials.json";
            NSDictionary *params = @{@"include_email": @"true", @"skip_status": @"true"};
            NSError *clientError;
            NSURLRequest *request = [client URLRequestWithMethod:@"GET" URLString:verifyCredentialsEndpoint parameters:params error:&clientError];
